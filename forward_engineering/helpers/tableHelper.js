...
const getCreateStatement = ({
	...
	return buildStatement(`CREATE${tempExtStatement}TABLE IF NOT EXISTS ${fullTableName} (`, isActivated)(
		columnStatement,
		indentString(columnStatement + (primaryKeyStatement ? ',' : '')),
	)(primaryKeyStatement, indentString(primaryKeyStatement))(foreignKeyStatement, indentString(foreignKeyStatement))(
		true,
		')',
	)(comment, `COMMENT '${comment}'`)(
		partitionedByKeys,
		commentStatementIfAllKeysDeactivated(`PARTITIONED BY (${partitionedByKeys.keysString})`, partitionedByKeys),
	)(
		hasClusteredBy,
		commentStatementIfAllKeysDeactivated(`CLUSTERED BY (${clusteredKeys.keysString})`, clusteredKeys),
	)(
		sortedKeys && hasClusteredBy,
		commentStatementIfAllKeysDeactivated(`SORTED BY (${sortedKeys.keysString})`, sortedKeys),
	)(hasClusteredBy, `INTO ${numBuckets} BUCKETS`)(skewedStatement, skewedStatement)(
		rowFormatStatement,
...
const getSortedKeys = (sortedKeys, deactivatedColumnNames, isParentItemActivated) => {
	...
	return {
		...
		keysString: `${getSortKeysStatement(activatedKeys)} /*, ${getSortKeysStatement(deactivatedKeys)} */`,
	};
};
...
const getPartitionKeyStatement = (keys, isParentActivated) => {
	...
	return {
		isAllKeysDeactivated: false,
		keysString: `${getKeysStatement(activatedKeys)} /*, ${getKeysStatement(deactivatedKeys)} */`,
	};
};
