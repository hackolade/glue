...
const parsePrimitive = ([type]) => {
	...
	const modifiers = get(preparedType.match(/\((.*?)\)$/), '[1]', '').split(',');
...
const parseUnion = (types, sample) => {
	...
	if (!complexTypes) {
		return {
			type: uniq(jsonSchemas.map(schema => schema.type)),
		};
	}
...
const getChoice = (jsonSchema, subSchemas, columnName) => {
	...
	if (jsonSchema['allOf']) {
		...
		jsonSchema['allOf'] = [{ oneOf: oldOneOf }, { oneOf }];
	}
